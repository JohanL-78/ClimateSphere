...
const TEXTURES_ANNUAL_DIR = path.join(TEXTURES_DIR, 'textures_annual');
...
async function getAvailableAnnualTextureYears() {
  try {
    const files = await fs.readdir(TEXTURES_ANNUAL_DIR);
    const lastCompleteYear = new Date().getFullYear() - 1;

    return files
      .map(file => file.match(/^gistemp_(\d{4})\.png$/)?.[1])
      .filter(Boolean)
      .map(Number)
      .filter(year => Number.isFinite(year) && year <= lastCompleteYear)
      .sort((a, b) => b - a);
  } catch (error) {
    console.warn(`Textures GISTEMP annuelles indisponibles: ${error.message}`);
    return [];
  }
}
...
export async function getAvailableDates(source = 'global', locale = 'en') {
  ...
  const annualTextureYears = await getAvailableAnnualTextureYears();
  ...
  return {
    years,
    annualYears: annualTextureYears.filter(year => dataYears.has(year)),
    months: monthOptionsByLocale[normalizedLocale],
    current_year,
    current_month,
    current_annual_year: annualTextureYears.find(year => dataYears.has(year)) || years.find(year => year <= new Date().getFullYear() - 1),
    textureMonthsByYear
  };
}
...
function buildRank(values, selectedValue) {
  const validValues = values.filter(Number.isFinite);
  if (!Number.isFinite(selectedValue) || validValues.length === 0) {
    return null;
  }

  return {
    rank: validValues.filter(value => value > selectedValue).length + 1,
    total: validValues.length
  };
}

function calculatePreindustrialAnomaly(parsedData, columnIndex, selectedValue) {
  if (!Number.isFinite(selectedValue)) return null;

  const preindustrialValues = parsedData
    .filter(row => {
      const year = Number(row[0]);
      return year >= 1880 && year <= 1899;
    })
    .map(row => parseFloat(row[columnIndex]))
    .filter(Number.isFinite);

  if (preindustrialValues.length === 0) return null;

  const preindustrialAverage = preindustrialValues.reduce((sum, value) => sum + value, 0) / preindustrialValues.length;
  return selectedValue - preindustrialAverage;
}

export async function getTemperatureArchiveStatsForDate(year, month, source = 'global') {
  const parsedData = await parseCSV(source);
  const monthIndex = parseInt(month, 10);
  ...
  const yearRow = parsedData.find(row => row[0] === String(year));
  const selectedValue = yearRow?.[monthIndex] === '' ? null : parseFloat(yearRow?.[monthIndex]);
  ...
  const sameMonthValues = parsedData
    .map(row => parseFloat(row[monthIndex]))
    .filter(Number.isFinite);

  const absoluteValues = parsedData
    .flatMap(row => row.slice(1, 13).map(cell => parseFloat(cell)))
    .filter(Number.isFinite);

  return {
    value: selectedValue,
    preindustrial: calculatePreindustrialAnomaly(parsedData, monthIndex, selectedValue),
    ranks: {
      sameMonth: buildRank(sameMonthValues, selectedValue),
      absolute: buildRank(absoluteValues, selectedValue)
    }
  };
}

export async function getAnnualTemperatureArchiveStatsForYear(year, source = 'global') {
  const parsedData = await parseCSV(source);
  const annualColumnIndex = 13;
  const yearRow = parsedData.find(row => row[0] === String(year));
  const selectedValue = yearRow?.[annualColumnIndex] === '' ? null : parseFloat(yearRow?.[annualColumnIndex]);
  ...
  const lastCompleteYear = new Date().getFullYear() - 1;
  const annualValues = parsedData
    .filter(row => Number(row[0]) <= lastCompleteYear)
    .map(row => parseFloat(row[annualColumnIndex]))
    .filter(Number.isFinite);

  return {
    value: selectedValue,
    preindustrial: calculatePreindustrialAnomaly(parsedData, annualColumnIndex, selectedValue),
    ranks: {
      annual: buildRank(annualValues, selectedValue)
    }
  };
}
